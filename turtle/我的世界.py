...
class World:
    ...
    def update(self, player_position: tuple[int, int]):
        ...
        math.floor(player_position[0] / CHUNK_LENGTH), math.floor(player_position[1] / CHUNK_LENGTH))
    ...
    if base_position[0] >= TREE_LEAVES_HALFWIDTH - 1 and base_position[0] < CHUNK_LENGTH - TREE_LEAVES_HALFWIDTH:
        if base_position[1] <= CHUNK_LENGTH - TREE_TOTAL_HEIGHT:
            ...
            for y_off in range(TREE_LEAVES_HEIGHT):
                ...
                for x_off in range(-TREE_LEAVES_HALFWIDTH + (y_off == TREE_LEAVES_HEIGHT - 1),
                                   TREE_LEAVES_HALFWIDTH + 1 - (y_off == TREE_LEAVES_HEIGHT - 1)):
...
def aabb_check(min1: tuple[float, float], max1: tuple[float, float], min2: tuple[float, float],
               max2: tuple[float, float]):
    """
